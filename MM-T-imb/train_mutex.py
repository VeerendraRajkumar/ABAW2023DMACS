...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = MutexMatch(_net_builder,
                     ...
                     args.ema_m,
                     args.T,
                     [args.p_cutoff]*args.num_classes, # args.p_cutoff,
                     args.ulb_loss_ratio,
                     args.hard_label,
                     args.k,
                     ...
                     tb_log=tb_log,
                     logger=logger, 
                     data = args.dataset, load=args.tl_path)
    ...
    logger.info(f"model_arch: {str(model)}")
    ...
    if args.dataset == 'aff':
        # @V&N[14 Mar 23]
        from datasets_affwild2.affwild2 import get_data_loaders
        data_para_dict = dict(
            dataset = args.dataset,
            n_lab_pc = args.num_labels,
            num_classes=args.num_classes,
            batch_size = args.batch_size,
            mu = args.uratio,
            iters = args.num_train_iter,
            num_workers=args.num_workers, 
        )

        dltrain_x, dltrain_u, dlval, dltrain_u_eval = get_data_loaders( **data_para_dict)
        loader_dict['train_lb'] = dltrain_x
        loader_dict['train_ulb'] = dltrain_u
        loader_dict['eval'] = dlval
        loader_dict['train_l_eval'] = dltrain_u_eval
    else:
    ...
    if args.tl_path == './models/nets/best_raf_model.pth':
        model.load_model_from_chkpth(args.tl_path)
    if args.resume:
...
if __name__ == "__main__":
    ...
    # This argument is sepcifically for loading the checkpoint 
    # for helping the model to learn better using transfer-learning(tl)
    parser.add_argument('--tl_path', type=str, default=None, help='path to initial checkpoint')
    ...
    parser.add_argument('--num_labels', type=int, default=195653,
                        help='Total number of labeled samples')
    # This argument is specifically used for RAF-DB dataset
    # parser.add_argument('--n_fear', type=int, default=250, 
    #                     help='number of samples used for fear class')
    ...
    '''
    parser.add_argument('--net', type=str, default='wrn', help='backbone net name to be used')
