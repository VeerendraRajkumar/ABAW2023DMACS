...
MutexMatch with threshold - utils
...
def net_builder(net_name, net_conf=None, num_classes=10):
    ...
    if net_name == 'wrn':
        ...
        # return builder.build(10, True)
    elif net_name == 'effnetb0':
        import models.nets.effnet as net
        base_net = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b0', pretrained=True)
        builder = getattr(net, 'build_EfficientNetb0')()
        setattr_cls_from_kwargs(builder, {'base_net':base_net})
    elif net_name == 'effnetb4':
        import models.nets.effnet as net
        base_net = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b4', pretrained=True)
        builder = getattr(net, 'build_EfficientNetb4')()
        setattr_cls_from_kwargs(builder, {'base_net':base_net})
    else:
