...
MutexMatch with threshold - resnet18 backbone
...
class ResNet18(nn.Module):
    def __init__(self, num_classes, base_net, load=None):
        super(ResNet18, self).__init__() 
        if load:
            self.backbone = base_net
        else:
...
class build_ResNet18:
    ...
    def build(self, num_classes, load=None):
        return ResNet18(num_classes = num_classes, base_net = self.base_net(), load=load) 
